...
const StakingHistory = () => {
  ...
  const fetchStakings = async () => {
    ...
    try {
      ...
      // First, check and process completed stakings
      const response = await fetch(`/api/staking/complete?userId=${user.id}`, {
        method: 'POST',
      });
      
      if (!response.ok) {
        throw new Error("Failed to process completed stakings");
      }

      // Then fetch updated staking data
      const stakingResponse = await fetch(`/api/staking?userId=${user.id}`);
      if (!stakingResponse.ok) {
        throw new Error("Failed to fetch staking data");
      }

      const data = await stakingResponse.json();
      console.log("Fetched staking data:", data);
                        ...
                        <div className="bg-[#172130] rounded-[16px] px-5 py-5">
                          ...
                          <div className="py-5">
                            <div className="rounded-full w-full overflow-hidden bg-[#29303A] h-[18px] relative">
                              <div
                                className="absolute left-0 rounded-full text-[12px] font-medium flex pr-2 text-[#3C3C3C] justify-end top-0 h-full"
                                style={{ 
                                  width: `${clampedPercentage}%`, 
                                  backgroundColor: status === "ACTIVE" ? "linear-gradient(to right, #0ABEF1, #0EF7BA)" : "#0EF7BA" 
                                }}
                              >
